...
// when node gets reparented, resize new and previous parent (and their parents recursively) to encapsulate children 
  ...
  const { screenToFlowPosition,flowToScreenPosition,isNodeIntersecting,getIntersectingNodes,fitBounds,getNodesBounds,getNode,updateNode} = useSvelteFlow();
  ...
  const getPositionOfOrigin = node => 
    subPositions(node.position,getOffsetOfOrigin(node))
...
<SvelteFlow bind:nodes bind:edges {colorMode} {nodeTypes} defaultEdgeOptions={edgeDefaults} fitView
